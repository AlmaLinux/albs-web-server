...
def parse_args(args):
    ...
    parser.add_argument('-p', '--package-links', nargs='+', required=True,
                        type=str)
...
async def async_main(args, work_dir, logger):
    ...
    for link in args.package_links:
        ...
        logger.debug('Check that such file already exists in Pulp: %s',
                     file_name)
    ...
    logger.debug('RPM hrefs to add to repository: %s',
                 pprint.pformat(rpm_package_hrefs, indent=4))

    await pulp_client.modify_repository(
        repository['pulp_href'], rpm_package_hrefs)
