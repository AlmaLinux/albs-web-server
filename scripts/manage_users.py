...
from alws.crud import actions as action_crud
from alws.crud import roles as role_crud
...
def parse_args():
    ...
    parser.add_argument(
        '-e', '--email', required=True, type=str, help='User e-mail'
    )
    parser.add_argument(
        '-t', '--team-name', required=False, type=str, help='Team name'
    )
    parser.add_argument(
        '-a',
        '--add-role',
    ...
    async with open_async_session(key=get_async_db_key()) as db:
        user = (
            (
                await db.execute(
                    select(models.User)
                    .where(models.User.email == arguments.email)
                    .options(
        ...
        if not user:
            raise ValueError(
                f'No such user in the system: ' f'{arguments.email}'
            )
        ...
        if arguments.superuser and arguments.usual_user:
            raise ValueError(
                'Cannot both make user a superuser ' 'and usual one'
            )
        ...
        if (
            arguments.add_roles or arguments.remove_roles
        ) and not arguments.team_name:
            ...
            (
                await db.execute(
                    select(models.Team).where(
                        models.Team.name == arguments.team_name
                    )
                )
            )
            .scalars()
        ...
        if not team:
            raise ValueError(
                f'No such team in the system: ' f'{arguments.team_name}'
            )
