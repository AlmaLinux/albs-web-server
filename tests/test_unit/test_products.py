...
group_tasks_by_ref_id_data_all_success = (
    [
        _create_build_task_mock(
            {"id": 1, "ref_id": 1, "status": BuildTaskStatus.COMPLETED}
        ),
        _create_build_task_mock(
            {"id": 2, "ref_id": 1, "status": BuildTaskStatus.COMPLETED}
        ),
        _create_build_task_mock(
            {"id": 3, "ref_id": 2, "status": BuildTaskStatus.COMPLETED}
        ),
        _create_build_task_mock(
            {"id": 4, "ref_id": 2, "status": BuildTaskStatus.COMPLETED}
        ),
    ],
    {1: [(1, True), (2, True)], 2: [(3, True), (4, True)]},
)

group_tasks_by_ref_id_data_refs_have_one_failed = (
    [
        _create_build_task_mock(
            {"id": 5, "ref_id": 3, "status": BuildTaskStatus.COMPLETED}
        ),
        _create_build_task_mock(
            {"id": 6, "ref_id": 3, "status": BuildTaskStatus.FAILED}
        ),
        _create_build_task_mock(
            {"id": 7, "ref_id": 4, "status": BuildTaskStatus.FAILED}
        ),
        _create_build_task_mock(
            {"id": 8, "ref_id": 4, "status": BuildTaskStatus.COMPLETED}
        ),
    ],
    {3: [(5, True), (6, False)], 4: [(7, False), (8, True)]},
)

group_tasks_by_ref_id_data_refs_one_failed_all = (
    [
        _create_build_task_mock(
            {"id": 9, "ref_id": 5, "status": BuildTaskStatus.COMPLETED}
        ),
        _create_build_task_mock(
            {"id": 10, "ref_id": 5, "status": BuildTaskStatus.FAILED}
        ),
        _create_build_task_mock(
            {"id": 11, "ref_id": 6, "status": BuildTaskStatus.FAILED}
        ),
        _create_build_task_mock(
            {"id": 12, "ref_id": 6, "status": BuildTaskStatus.FAILED}
        ),
    ],
    {5: [(9, True), (10, False)], 6: [(11, False), (12, False)]},
)
...
build = {
    ...
    "tasks": [
        ...
        {"id": 3, "url": "https://build.task.ref#3"},
        {"id": 4, "url": "https://build.task.ref#4"},
        {"id": 5, "url": "https://build.task.ref#5"},
        {"id": 6, "url": "https://build.task.ref#6"},
    ],
    "linked_builds": [],
    "is_secure_boot": True,
    "mock_options": {},
...
class TestProductsUnit(BaseAsyncTestCase):
    ...
    async def test_get_packages_to_blacklist(
        self, tasks_and_expected_output, session
    ):
