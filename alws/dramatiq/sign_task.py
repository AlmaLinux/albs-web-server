...
async def _complete_gen_key_task(
        task_id: int,
        payload: typing.Dict[str, typing.Any],
):
    await sign_task.complete_gen_key_task(
        task_id,
        sign_schema.GenKeyTaskComplete(**payload),
    )
...
@dramatiq.actor(
    ...
    priority=1,
    queue_name='sign',
    time_limit=DRAMATIQ_SIGN_TASK_TIMEOUT,
)
def complete_sign_task(
        task_id: int,
        payload: typing.Dict[str, typing.Any],
):
...
@dramatiq.actor(
    max_retries=2,
    priority=1,
    queue_name='gen_sign_key',
    time_limit=DRAMATIQ_GEN_KEY_TASK_TIMEOUT,
)
def complete_gen_key_task(
        task_id: int,
        payload: typing.Dict[str, typing.Any],
):
    event_loop.run_until_complete(_complete_gen_key_task(task_id, payload))
