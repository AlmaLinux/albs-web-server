...
class PulpClient:
    ...
    async def upload_file(
        self, content=None, file_path: str = None, sha256: str = None
    ):
    ...
    async def create_rpm_publication(self, repository: str, sleep_time: float = 10.):
    ...
    async def sync_rpm_repo_from_remote(
        ...
        payload = {"remote": remote_href, "sync_policy": sync_policy}
        task = await self.request("POST", endpoint, json=payload)
        if wait_for_result:
            result = await self.wait_for_task(task["task"])
            return result
    ...
    async def update_filesystem_exporter(
        ...
        params = {"name": fse_name, "path": fse_path, "method": fse_method}
    ...
    async def export_to_filesystem(
        self, fse_pulp_href: str, fse_repository_version: str
    ):
        endpoint = urllib.parse.urljoin(fse_pulp_href, "exports/")
        params = {"repository_version": fse_repository_version}
    ...
    async def get_rpm_publications(
        self,
        repository_version_href: str = None,
        include_fields: typing.List[str] = None,
        exclude_fields: typing.List[str] = None,
    ):
        endpoint = "pulp/api/v3/publications/rpm/rpm/"
    ...
    async def wait_for_task(self, task_href: str, sleep_time: float = 5.):
        ...
        if task["state"] == "failed":
            ...
            if error:
                ...
                if traceback:
                    error_msg += f"\nPulp traceback:\n{traceback}"
            raise Exception(f"Task {task['pulp_href']} has failed{error_msg}")
        return task
