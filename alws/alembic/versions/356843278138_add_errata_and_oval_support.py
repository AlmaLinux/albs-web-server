"""Add errata and oval support

Revision ID: 356843278138
Revises: 433fd427f579
Create Date: 2022-05-13 10:55:36.600159
...
def upgrade():
    ...
    op.create_table('errata_records',
    sa.Column('id', sa.Text(), nullable=False),
    ...
    sa.Column('original_criteria', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
    sa.Column('tests', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.Column('original_tests', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
    sa.Column('objects', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.Column('original_objects', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
    sa.Column('states', postgresql.JSONB(astext_type=sa.Text()), nullable=True),
    sa.Column('original_states', postgresql.JSONB(astext_type=sa.Text()), nullable=False),
    sa.ForeignKeyConstraint(['platform_id'], ['platforms.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('errata_packages',
    ...
    sa.ForeignKeyConstraint(['errata_record_id'], ['errata_records.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('errata_references',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('href', sa.Text(), nullable=False),
    sa.Column('ref_id', sa.Text(), nullable=False),
    sa.Column('title', sa.Text(), nullable=False),
    sa.Column('ref_type', sa.Enum('cve', 'rhsa', 'self_ref', 'bugzilla', name='erratareferencetype'), nullable=False),
    sa.Column('errata_record_id', sa.Text(), nullable=False),
    sa.Column('cve_id', sa.Text(), nullable=True),
    sa.ForeignKeyConstraint(['cve_id'], ['errata_cves.id'], ),
    sa.ForeignKeyConstraint(['errata_record_id'], ['errata_records.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_table('errata_to_albs_packages',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    sa.Column('status', sa.Enum('proposal', 'skipped', 'released', 'approved', name='erratapackagestatus'), nullable=False),
    sa.ForeignKeyConstraint(['albs_artifact_id'], ['build_artifacts.id'], ),
    sa.ForeignKeyConstraint(['errata_package_id'], ['errata_packages.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.add_column('platforms', sa.Column('contact_mail', sa.Text(), nullable=True))
    op.add_column('platforms', sa.Column('copyright', sa.Text(), nullable=True))
    # ### end Alembic commands ###


def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_column('platforms', 'copyright')
    op.drop_column('platforms', 'contact_mail')
