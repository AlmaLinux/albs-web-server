...
RUN cd /code && virtualenv -p python3.9 --system-site-packages env && source env/bin/activate && pip3 install -U pip
RUN cd /code && source env/bin/activate && pip3 install -r /tmp/requirements.txt --no-cache-dir
COPY alws /code/alws
